...
public class FishSpawnList {
        ...
        for (int i = 0; i < pokemonList.size(); i++) {
            ...
            if (name.contains("-")) {
                ...
                if (name.equalsIgnoreCase("porygon-z")) {
                    ...
                    pokemon = PokemonSpec.from(name).create(player.world);
                    int pokemonForm = GenerationsFormIndexFromName.getFormNumberFromFormName(name, form);
    ...
    private Button getPokemonSprite(String name, FishingSpawnInfo info) {
        ...
        if (GenerationsHeldItemUtils.heldItemMap.containsKey(name)) {

            Map<String, List<String>> possibleItems = null;
            for (Map.Entry<String, Map<String, List<String>>> entry : GenerationsHeldItemUtils.heldItemMap.entrySet()) {
